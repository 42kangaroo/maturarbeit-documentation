...
\]
For $c \cdot r(n) > \log(t(n))$ for some fixed $c$, we can do the same counting trick.
...
The same method also means that $t(n)r(n) \geq s(n)$ because of Savitch's Theorem and the Space Hierearchy theorem (\cref{subsec:space-hierarchy-theorem}).
...
Here, we define FO$\forall, \exists$-VAR$[t(n), f(n), s(n)]$ to contain all formulas wich have $t(n)$ iterations, extended variables of $f(n)$ bits which can be universally quantified and $s(n)\log(n)$ bit extended variables which can be used in existential quantification.
