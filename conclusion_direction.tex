...
This has led us to have a better notion of how all these logics are related to NSPACE$[s(n)]$ and thus also to linear-bounded automata and context-sensitive langauges.
...
This approach does not contain any new insights, but is a good showcase of the close corelation between first- and second-order logic.
...
A full proof was impossible to make as it is unclear how to generalise this direction of work.
The difficulty of this problem is again illustrated by Savitch's Theorem, which has not been improved for a long time and thus makes it seem unlikely that a simulation of NSPACE machines with nonquandratic DSPACE machines is possible.
...
The generalisation of this idea by adding more bits to the extended variables and decreasing the iteration count gives an interesting space-time tradeoff, but does not solve the inherent problem of simulating this formula by NSPACE machines.
...
\begin{itemize}
    ...
    \item First of all, I often feel that my proofs are not explained in a way that is clear and concise.
...
This is still a quite profound limitation, and no characterisation for superpolynomial bounds is known to me.
Finding a generalisation could lead to new insights that would help in the polynomial case and in the end to the case where $s(n) = n$ and we have exactly the context-sensitive languages.
...
After showing that mixing TC and iterative procedures does not give any strong characterisations in \cref{sec:mixing-iterations-and-transitive-closure} and seeing that all interesting cases happen when the extended variables have less than $s(n)$ bits, one direction that took up most of the time and yielded very little results came up.
...
To do this I read multiple papers concerning hierarchies over the transitive closure, Generalised quantifiers and alternating space-time.
...
The main problem with these approaches is that they can not be generalised to include string structures, which are inherently ordered.
